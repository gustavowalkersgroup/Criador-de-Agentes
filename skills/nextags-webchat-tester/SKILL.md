...
description: "Testa um agente de webchat NexTags (plataforma tapthetable) AO VIVO, dirigindo o WebSocket do webchat por Python — sem extensão do Chrome. Exercita a stack REAL publicada (modelo NexTags + MCP + APIs de backend tipo Nuvemshop/Bling/Shopify). Use quando quiser TESTAR/VALIDAR/conversar com o bot PUBLICADO via webchat, confirmar que uma mudança de prompt/MCP funcionou na infra real, simular conversa de cliente, ou debugar roteamento/handoff/transferência ao vivo. Gatilhos PT-BR: 'testar o bot no webchat', 'conversar com a IA ao vivo', 'validar o atendente no webchat', 'o bot respondeu certo?', 'testa o agente publicado'. EN: 'test the nextags bot via webchat', 'drive the live webchat'. Diferente de simular o prompt em contexto: passa pelo NexTags + MCP + backend de verdade (pega erro de credencial MCP, payload grande, renderização de card, handoff, transferência fantasma)."
metadata:
  type: reference
---
